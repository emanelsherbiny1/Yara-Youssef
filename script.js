document.addEventListener("DOMContentLoaded", () => {
    ...
    window.openInvitation = function() {
        ...
        setTimeout(() => {
            ...
            // Mobile-safe audio: triggered by the user's tap that opened the invitation
            ...
            audio.play().then(() => {
                ...
                // Mobile blocked autoplay — FAB stays paused, user taps to start
                musicPlaying = false;
    ...
    // ── RSVP FORM — WhatsApp ─────────────────────
    window.submitRsvpWhatsApp = function() {
        ...
        if (!attending) { alert('Please select if you will attend.'); return; }

        const attendingText = attending.value === 'yes' ? '✅ Joyfully Accepts' : '❌ Regretfully Declines';
        const wish = message ? `\n💌 Wish: ${message}` : '';

        const text = encodeURIComponent(
            `🌸 *RSVP — Yara & Youssef Wedding*\n` +
            `👤 Name: ${name}\n` +
            `${attendingText}\n` +
            `👥 Guests: ${guests}` +
            wish
        ...
        window.open(`https://wa.me/201010288153?text=${text}`, '_blank');
